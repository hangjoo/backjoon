...
문자열 T와 P가 주어졌을 때, T의 부분 문자열과 P가 일치하는 횟수와 그 때의 인덱스를 구하는 문제입니다.

문제에서 주어진 알고리즘은 KMP 알고리즘으로 해당 알고리즘을 통해 구현하는 문제입니다. KMP 알고리즘에 관한 설명은 잘 설명해준 블로그가 있어 해당 링크로 대체하겠습니다.

KMP 알고리즘을 사용하여 주어진 문자열 T의 부분 문자열이 문자열 P와 일치하는 인덱스를 ans 리스트 담아 계산하고 이를 출력하는 방법으로 문제를 해결할 수 있습니다.

처음 접해보는 알고리즘이라 문제를 푸는 것보다 알고리즘을 이해하는데 많은 시간이 걸린 문제였습니다. 문자열의 경우 코딩 테스트에서 자주 등장하는 문제이기 때문에 알아두면 좋을 것 같습니다.